...
class Model(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.model = torch.hub.load(
            'rwightman/gen-efficientnet-pytorch', 'efficientnet_b0', pretrained=True)  # "mixnet_l"
    ...
    def prepare_data(self):
        ...
        train_transforms = transforms.Compose([
            ...
            transforms.ColorJitter(
                brightness=.12, contrast=.035, saturation=.035),
            transforms.RandomAffine(
                50, scale=(.85, 1.15), shear=(5)),
            transforms.ToTensor(),
            transforms.Normalize(mean=[.485, .456, .406],
                                 std=[.229, .224, .225]),
            transforms.RandomErasing(p=.3),
        ])
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=64-16, shuffle=True, drop_last=False, num_workers=4)
